...
You can even gather and use facts about one system when configuring another system. For example, you can get the IP address from your DB server and use it when configuring a DB client.

## Disabling fact gathering

Sometimes you don't need to gather facts. Be careful though, even if you don't use them directly, your Galaxy roles or built-in tasks might rely on them. But there are some cases when you can be 100% sure you won't need the facts. For example, when working with clouds you then specify localhost as your playbook's target, but it's only the host that runs the cloud API client, you don't really care about its configuration. You can then begin your playbook with:
...
If you want to make your playbooks run a few seconds faster, there's a better way than disabling fact gathering: fact caching. For more complex setups you can use the DB, but a simple file is usually enough. Add this to your ansible.cfg:
