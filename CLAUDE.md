...
- Sections (`mainSections` in `hugo.toml`): `general`, `home`, `internet`, `homelab`.
  ...
  - `homelab` - experimentation/tinkering for its own sake, not production self-hosting (Kubernetes on Minikube, GPU/CUDA experiments, security tooling like Wazuh) - things run to learn, not because the home network needs them.
